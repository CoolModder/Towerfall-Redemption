...
public class LandMine : Actor
{
    ...
    public LandMine(Vector2 position, float rotation) : base(position)
    {
        Position = position;
        base.Depth = 150;
        Tag(GameTags.PlayerCollider, GameTags.ExplosionCollider, GameTags.ShockCollider, GameTags.LavaCollider, GameTags.Target);
        ScreenWrap = true;
        Pushable = false;
        base.Collider = new WrapHitbox(8f, 8f, -4f, -4f);
        image = new FlashingImage(ExampleModModule.LandMineAtlas["LandMine"]);
        image.CenterOrigin();
